...
COPY deploy/aws-cloudwatch-config/amazon-cloudwatch-agent-prod.json /opt/aws/amazon-cloudwatch-agent/etc/amazon-cloudwatch-agent.json
# CollectD file setting for AWS Cloudwatch
RUN sudo mkdir -p /usr/share/collectd
RUN sudo touch /usr/share/collectd/types.db
